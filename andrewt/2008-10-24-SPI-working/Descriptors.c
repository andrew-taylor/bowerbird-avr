...
USB_Descriptor_Configuration_t ConfigurationDescriptor PROGMEM =
  { Config:
    ...
    { Header:			{ Size: sizeof(USB_AudioFeatureUnit_t),
      ...
      UnitID:			0x02,
      SourceID:			0x01,
      ControlSize:              AUDIO_CHANNELS, // number of elts in the ChannelControls array
      MasterControls:           0x0000, // FEATURE_VOLUME, // FEATURE_INPUT_GAIN_CONTROL, // FIXME master, applies to all channels.
      ChannelControls:          { FEATURE_INPUT_GAIN_CONTROL, FEATURE_INPUT_GAIN_CONTROL }, // FIXME per-channel controls, one entry per channel
    ...
    { Header:                   { Size: sizeof(USB_AudioOutputTerminal_t),
      ...
      SourceID:                 0x02, // source from the feature unit?
      TerminalStrIndex:         NO_DESCRIPTOR_STRING			
    },
    ...
    { Header:                   {Size: sizeof(USB_AudioFormat_t), Type: DTYPE_AudioInterface},
      ...
      SubFrameSize:             0x02, // 2 bytes per sample FIXME verify: should this be 2 * channels?
	...
	Attributes:		(EP_TYPE_ISOCHRONOUS | ENDPOINT_ATTR_ASYNC), // | ENDPOINT_USAGE_DATA), -- FIXME 0x05
