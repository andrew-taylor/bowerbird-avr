...
ISR(TIMER0_COMPA_vect, ISR_BLOCK)
{
  ...
  Endpoint_Write_Word_LE(val * gain[AUDIO_CHANNELS - 1] * 64);
...
#endif
  /* If the endpoint is full, flush it. It is double-banked, i.e. we
   * can fill up the other one while the host reads from the current
   * one. */
  if(Endpoint_BytesInEndpoint() > AUDIO_STREAM_EPSIZE - (AUDIO_CHANNELS * sizeof(uint16_t))) {
